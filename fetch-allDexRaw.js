...
const CONFIG = {
    ...
    tokenListFile: './solana_tokens.json',
    tokenListUrl: 'https://raw.githubusercontent.com/allanpark/solana-token-list/main/src/tokens/solana.tokenlist.json',
    fetchCount: {
        raydiumCPMM: 40,
        raydiumCLMM: 40,
        orca: 40,
...
// Global Mint Map (populated from token list + known mints)
...
const KNOWN_MINTS = {
    ...
    '27G8Mt6P9b8F4Jjy3GXBC3GCaXLbJQkFHFSATFjFJ8b': { decimals: 6, symbol: 'jitoSOL' },
    'DezXAZ8z7PnrnRJjz3wXBoRgixCa6xjnB7YaB1pPB263': { decimals: 5, symbol: 'BONK' },
    '3NZ9JMVBmGAqocybic2c7LQCJScmgsAZ6vQqTDzcqmJh': { decimals: 8, symbol: 'WBTC' },
    'cbbtcf3aa214zXHbiAZQwf4122FBYbraNdFqgw4iMij': { decimals: 8, symbol: 'cbBTC' },
};

async function loadTokenList() {
    // 1. Load known mints first
    for (const [mint, data] of Object.entries(KNOWN_MINTS)) {
        MINT_MAP.set(mint, data);
    }

    // 2. Download list if missing
    if (!fs.existsSync(CONFIG.tokenListFile)) {
        try {
            console.log('  Downloading token list...');
            const res = await axios.get(CONFIG.tokenListUrl, { timeout: 10_000 });
            fs.writeFileSync(CONFIG.tokenListFile, JSON.stringify(res.data, null, 2));
            console.log('  ✅ Token list downloaded.');
        } catch (e) {
            console.warn(`  ⚠️  Failed to download token list: ${e.message}`);
        }
    }
    ...
    if (fs.existsSync(CONFIG.tokenListFile)) {
        try {
            const content = fs.readFileSync(CONFIG.tokenListFile, 'utf8');
            const json = JSON.parse(content);
            const tokens = json.tokens || [];
            console.log(`  Loaded ${tokens.length} tokens from ${CONFIG.tokenListFile}`);

            for (const t of tokens) {
                // Don't overwrite known mints if they exist (allows manual overrides)
                if (!MINT_MAP.has(t.address)) {
                    MINT_MAP.set(t.address, {
                        decimals: t.decimals,
                        symbol: t.symbol
                    });
                }
            ...
            console.warn(`  ⚠️  Failed to load token list: ${e.message}`);
        }
    }
}

function inferDecimals(mint) {
    return MINT_MAP.get(mint)?.decimals ?? 6;  // default 6 for unknown SPL tokens
}

function inferSymbol(mint) {
    return MINT_MAP.get(mint)?.symbol ?? '';
}
...
// RAYDIUM CPMM
//
// Fixed endpoint: /v2/main/pairs (V3 is 500)
// TVL  : pool.liquidity (USD)
// Price: pool.price
// Note : tokenAmountCoin/Pc are DECIMAL human amounts.
// ─────────────────────────────────────────────────────────────────────────────
async function fetchRaydiumCPMM() {
    const url = 'https://api.raydium.io/v2/main/pairs';
    const data = await fetchWithRetry(url);
    // V2 returns array directly, not data.data
    ...
    for (const pool of pools) {
        ...
        const feeRate = 0.0025; // Standard CPMM fee
        const decA = inferDecimals(pool.baseMint);
        const decB = inferDecimals(pool.quoteMint);

        // mintAmountA/B are decimal → convert to raw integer reserves
        const xReserve = safeBigInt(parseFloat(pool.tokenAmountCoin || 0) * Math.pow(10, decA));
        const yReserve = safeBigInt(parseFloat(pool.tokenAmountPc || 0) * Math.pow(10, decB));
        ...
        results.push({
            ...
            vaults: {
                xVault: pool.baseVault || '',
                yVault: pool.quoteVault || '',
            },
            raw: pool,
        });
    }
...
// RAYDIUM CLMM
//
// Fixed endpoint: /v2/ammV3/ammPools (V3 is 500)
// ─────────────────────────────────────────────────────────────────────────────
async function fetchRaydiumCLMM() {
    const url = 'https://api.raydium.io/v2/ammV3/ammPools';
    ...
    for (const pool of pools) {
        ...
        let feeRate = 0;
        if (pool.ammConfig?.tradeFeeRate != null) feeRate = pool.ammConfig.tradeFeeRate / 1_000_000;

        const decA = pool.mintDecimalsA ?? 9;
        const decB = pool.mintDecimalsB ?? 6;

        // V2 CLMM API does NOT return reserve amounts. 
        // We set to '0' and let enricher fetch real on-chain balances.
        const xReserve = 0n;
        const yReserve = 0n;

        // Extract symbols from our map since API doesn't provide them
        const baseSymbol = inferSymbol(pool.mintA);
        const quoteSymbol = inferSymbol(pool.mintB);
        ...
        results.push({
            ...
            feeRate,
            feeBps: Math.round(feeRate * 10_000),
            tickSpacing: pool.ammConfig?.tickSpacing || 60,
            price: parseFloat(pool.price || 0),
            xReserve: xReserve.toString(),
            yReserve: yReserve.toString(),
...
//. node fetcher/fetch-allDexRaw.js